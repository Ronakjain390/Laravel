...
<body>
    <div class="email-container">
        ...
        <div class="content">
            <p>Hello, {{ ucfirst($challan->receiver)  }}</p>
            <p>A new comment "{{$status_comment}}" is added by {{ $challan->sender }} to {{ $heading}} Number: {{ strtoupper($challan->challan_series) }}-{{ $challan->series_num }}</p>
            <p>Please login to your <a href="https://theparchi.com/">theparchi.com</a> account to take further action.</p>
            {{-- <div class="button-container">
                <a href="{{ Storage::disk('s3')->temporaryUrl($challan->pdf_url, now()->addMinutes(5)) }}" class="button" style="color: #ffffff">View Challan</a>
            </div> --}}
