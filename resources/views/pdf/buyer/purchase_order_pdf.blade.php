...
<head>
    ...
    <style>
        ...
        .purchaseOrder-box table {
            width: 100%;
            line-height: inherit;
            text-align: left;
        }

        .purchaseOrder-box table td {
            /* padding: 5px; */
            vertical-align: top;
        }

        .purchaseOrder-box table tr td:nth-child(2),
        .text-right {
            /* text-align: right; */
        }

        .purchaseOrder-box table tr.top table td {
            padding-bottom: 20px;
        }

        .purchaseOrder-box table tr.top table td.title {
            font-size: 45px;
            line-height: 45px;
            color: #333;
        }

        .purchaseOrder-box table tr.information table td {
            padding-bottom: 20px;
            font-size: smaller;
            line-height: 12px;
        ...
        .purchaseOrder-box table tr.heading td {
            background: black;
            color: white;
            margin: 0;
            font-size: 12px;
        ...
        .purchaseOrder-box table tr.details td {
            padding-bottom: 20px;
        }
        ...
        .purchaseOrder-box table tr.item.last td {
            border-bottom: none;
        }

        .purchaseOrder-box table tr.total td:nth-child(2) {
...
// dd($purchaseOrder);
    @endphp
      ...
      @if ($pdfData && isset($pdfData->po_alignment)) @if ($pdfData->po_alignment == 'center')
              center-align
          @elseif($pdfData->po_alignment == 'left')
              left-align
          @elseif($pdfData->po_alignment == 'right')
              right-align @endif
      @endif">
             <div>
                <table>
                    <tbody>
                        <tr>
                            <td colspan="2">
                                <h1>{{ $pdfData->po_heading ?? 'Purchase Order' }}</h1>
                                {{ strtoupper($purchaseOrder->purchase_order_series) }}-{{ $purchaseOrder->series_num }}
                            </td>
                            <td></td>
                            <td style="width: 30%; text-align: right; padding-top:10px">
                                <br> <br>
                                <b>Date: {{ date('j-m-Y', strtotime($purchaseOrder->statuses[0]->created_at)) }}</b>
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            <table style="border-top: 1px solid;">
                                <tr>
                                    <td style="border-right: 1px solid #000; text-align: left;">
                                        <b> SENDER:</b>
                                        {{ ucfirst($purchaseOrder->sellerUser->company_name ?? $purchaseOrder->sellerUser->name) }}
                                        <br>
                                        @if (isset($purchaseOrder->sellerUser->address)) <b>Address:</b> {{
                                        ucfirst($purchaseOrder->sellerUser->address) }} <br> @endif

                                        @if (isset($purchaseOrder->sellerUser->city)) <b>City:</b> {{
                                        $purchaseOrder->sellerUser->city }} <br> @endif
                                        @if (isset($purchaseOrder->sellerUser->pincode)) <b>Pin Code:</b> {{
                                        $purchaseOrder->sellerUser->pincode }} <br> @endif
                                        @if (isset($purchaseOrder->sellerUser->state)) <b>State:</b> {{
                                        ucfirst($purchaseOrder->sellerUser->state) }} <br> @endif
                                        @if (isset($purchaseOrder->sellerUser->phone)) <b>Phone:</b> +91 {{
                                        $purchaseOrder->sellerUser->phone }} <br> @endif
                                        @if (isset($purchaseOrder->sellerUser->pancard))
                                        PAN: {{ strtoupper($purchaseOrder->sellerUser->pancard) }} <br>
                                        @endif
                                        @if (isset($purchaseOrder->sellerUser->gst_number))
                                        GSTIN: {{ strtoupper($purchaseOrder->sellerUser->gst_number) }} <br>
                                        @endif
                                    ...
                                    <td style="border-right: 1px solid gray; text-align: left;">
                                        ...
                                        @if (isset($purchaseOrder->buyerUser->name))
                                        <b> RECEIVER </b> {{ ucfirst($purchaseOrder->buyerUser->name) }}<br>
                                        @elseif (isset($purchaseOrder->buyer))
                                        <b> RECEIVER </b> {{ ucfirst($purchaseOrder->buyer ) }}<br>
                                        @else
                                        <b> RECEIVER </b> Default<br>
                                        @endif
                                        @if (isset($purchaseOrder->buyerUser->address)) <b> Address: </b> {{
                                        ucfirst($purchaseOrder->buyerUser->address) }}<br> @endif
                                        @if (isset($purchaseOrder->buyerUser->city)) <b>City:</b> {{
                                        ucfirst($purchaseOrder->buyerUser->city) }} <br> @endif
                                        @if (isset($purchaseOrder->buyerUser->pincode)) <b>Pincode:</b> {{
                                        ucfirst($purchaseOrder->buyerUser->pincode) }} <br> @endif
                                        @if (isset($purchaseOrder->buyerUser->state)) <b>State: </b> {{
                                        ucfirst($purchaseOrder->buyerUser->state) }} <br> @endif
                                        @if (isset($purchaseOrder->buyerUser->phone)) <b>Phone: </b>+91 {{
                                        $purchaseOrder->buyerUser->phone }} <br> @endif
                                        @if (isset($purchaseOrder->buyerUser->email)) <b>Email: </b>{{
                                        $purchaseOrder->buyerUser->email }}<br> @endif
                                    </td>
            ...
            <table style="width: 100%; border-bottom:1px solid!;  text-align: right; font-size: xx-small; line-height: normal;" >
                ...
                @foreach($purchaseOrder->orderDetails as $index => $orderDetail)
                @php
                ...
                @if(count($taxTotals) > 0)
                    ...
                    @endif
                    @if(isset($purchaseOrder->buyerUser->state) && isset(Auth::user()->state) && strtoupper($purchaseOrder->buyerUser->state) === strtoupper(Auth::user()->state))
                        <tr>
            ...
            <table style="width: 100%; margin-top: 5px;">
                <thead>
                    <tr>
                        <td>
                            <b>Total Qty:</b>
                            <p>{{ $purchaseOrder->total_qty }}</p>
                        </td>
                    </tr>
                    @if (!empty($purchaseOrder->comment))
                    <tr>
                        <td colspan="2">
                            <b>Comment:</b>
                    ...
                    <tr class="total">
                        <td colspan="2">
                            ...
                            <br>
                            {{ numberToIndianRupees($purchaseOrder->total) }}
                            @endif
                        </td>
                        <td style="width: 30%; text-align: right;">
                            @if ($purchaseOrder->round_off != 0)
                            <b>Round Off</b>
                                <p>&#8377; {{$purchaseOrder->round_off}}</p>
                            @endif
                            <br>
                            @if ($showAmount)
                            <b> Grand Total </b>
                            ...
                            @endif
                            <br>
                            @if ($pdfData && $pdfData->signature_option_buyer === 'Signature' && isset($pdfData->signature_buyer))
                            <img style="height: 70px" src="{{ Storage::disk('s3')->temporaryUrl($pdfData->signature_buyer, now()->addHours(1)) }}" alt="">
                            <br>
                                ...
                                <tbody>
                                    <tr>
                                        <td style="font-size: smaller;">
                                            @if (isset($purchaseOrder->sellerUser->bank_name))
                                            <b>BANK DETAILS</b>
                                            <br>
                                            <b>BANK NAME : </b> {{$purchaseOrder->sellerUser->bank_name}}
                                            <br>
                                            <b>BRANCH : </b> {{$purchaseOrder->sellerUser->branch_name}}
                                            <br>
                                            <b>ACCOUNT NO :</b> {{$purchaseOrder->sellerUser->bank_account_no}}
                                            <br>
                                            <b>IFSC CODE :</b> {{$purchaseOrder->sellerUser->ifsc_code}}
                                            @endif
                                        </td>
                                    </tr>
                                    @if (isset($purchaseOrder->sellerUser->bank_name) && ($termsAndConditions && count($termsAndConditions) > 0))
                                    <tr>
                                        <td>
        ...
        <div class="footer">
            ...
            <div style="font-size: 10px;">
                @if ($pdfData && $pdfData->signature_option_buyer === 'FooterStamp')
                    * This is a computer-generated {{ $pdfData->po_heading ?? 'Purchase Order' }} and does not require a physical signature
                @endif
            </div>
            <br>
