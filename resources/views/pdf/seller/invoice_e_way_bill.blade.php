            ...
            <div>
                <table>
                    <tbody>
                        <tr>
                            <td colspan="2">
                                <h1>{{ $pdfData->invoice_heading ?? 'Invoice' }}</h1>
                                #{{ strtoupper($invoice->invoice_series) }}-{{ $invoice->series_num }} <br>
                                <b>Date: {{ date('j-m-Y', strtotime($invoice->statuses[0]->created_at)) }}</b>
                            </td>
                            <td></td>
                            ...
                            <table style="border-top: 1px solid;">
                                <tr>
                                    <td style="border-right: 1px solid #000; text-align: left;">
                                        ...
                                        {{ ucfirst($invoice->sellerUser->company_name ?? $invoice->sellerUser->name) }}
                                        <br>
                                        ...
                                        PAN: {{ strtoupper($invoice->sellerUser->pancard) }} <br>
                                        @endif
                                        @if (isset($invoice->sellerUser->gst_number))
                                        GSTIN: {{ strtoupper($invoice->sellerUser->gst_number) }} <br>
                                        @endif
                                    ...
                                    <td style="border-right: 1px solid gray; text-align: left;">
                                        ...
                                        @elseif (isset($invoice->buyer))
                                        <b> Bill To </b> {{ ucfirst($invoice->buyer ) }}<br>
                                        @else
                                        <b> Bill To </b> Default<br>
                                        @endif
                                        @if (isset($invoice->buyerUser->address)) <b> Address: </b> {{
                                        ucfirst($invoice->buyerUser->address) }}<br> @endif
                                        @if (isset($invoice->buyerUser->city)) <b>City:</b> {{
                                        ucfirst($invoice->buyerUser->city) }} <br> @endif
                                        @if (isset($invoice->buyerUser->pincode)) <b>Pincode:</b> {{
                                        ucfirst($invoice->buyerUser->pincode) }} <br> @endif
                                        @if (isset($invoice->buyerUser->state)) <b>State: </b> {{
                                        ucfirst($invoice->buyerUser->state) }} <br> @endif
                                        @if (isset($invoice->buyerUser->phone)) <b>Phone: </b>+91 {{
                                        $invoice->buyerUser->phone }} <br> @endif
                                        @if (isset($invoice->buyerUser->email)) <b>Email: </b>{{
                    ...
                    @endphp
                                 <tr>
                                    <td style="text-align: right; margin-top:5px;">
                                        {{-- @dd($invoice->calculate_tax,$totals, array_key_exists('totalWithoutTax', $totals) ? $totals['totalWithoutTax'] : 'N/A' ); --}}
                                        @if (isset($invoice->calculate_tax  ) && !empty($invoice->calculate_tax) && $invoice->calculate_tax == 1 )
                                        <td>
                                ...
                                @endif
                                    ...
                                    @elseif(isset($invoice->buyerUser->state) == false )
                                        <tr>
                                            <td><td> <td>CGST at {{ ((float)$tax )/2}}% : {{ number_format(($taxableAmount * $taxPercent) / 2, 2) }}</td></td></td>
                                        </tr>
                                        <tr>
                                ...
                                @else
                                @php
                                $withoutTax = number_format(array_key_exists('totalWithoutTax', $totals) ? floatval($totals['totalWithoutTax']) : 0, 2);
                                $discount = is_numeric($discount) ? $discount : 0; // Ensure $discount is numeric, default to 0 if not
                                $discountAmount = number_format($discount / 100 * floatval($withoutTax), 2);
                                $netSales = number_format(floatval($withoutTax) - floatval($discountAmount), 2);
                                @endphp
                                ...
                                <tbody>
                                    {{-- <tr>
                                        <td style="font-size: smaller;">
                                            @if (isset($invoice->buyerUser->bank_name))
                                            <b>BANK DETAILS</b>
                                            <br>
                                            <b>BANK NAME : </b> {{$invoice->buyerUser->bank_name}}
                                            <br>
                                            <b>BRANCH : </b> {{$invoice->buyerUser->branch_name}}
                                            <br>
                                            <b>ACCOUNT NO :</b> {{$invoice->buyerUser->bank_account_no}}
                                            <br>
                                            <b>IFSC CODE :</b> {{$invoice->buyerUser->ifsc_code}}
                                            @endif
                                        </td>
                                    </tr> --}}
                                    <tr>
