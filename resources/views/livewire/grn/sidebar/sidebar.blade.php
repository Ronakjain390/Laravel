<div class="min-h-screen px-2 overflow-y-auto border-neutral-400 border-r flex flex-col justify-between">
    ...
    <ul class="space-y-2 ">
        ...
        {{-- @foreach ($templates as $template)
            ...
            <li>
                @if ($template->template_page_name == 'create-goods-receipt' && $PlanFeatureUsageRecordResponse !== 'active')
                    <!-- Disabled Panel -->
                    
                    <div class="flex drop-shadow-lg text-sm px-2 text-black font-normal py-2 bg-gray-400 border-gray-300 border rounded-lg items-center group"  @click="open = ! open">
                        <span>{{ $template->template_name }}</span>
                    
                    </div>
                    
                @else
                    <!-- Active/Clickable Panel -->
                    <a href="{{ route('grn', ['template' => $template->template_page_name]) }}" class="flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100 rounded-lg items-center dark:focus:ring-gray-500 group">
                        <span>{{ $template->template_name }}</span>
                    </a>
                @endif
            </li>
            <div class="text-red-600 text-xs  " x-show="open" >Plan Expired! <a class="hover:underline" href="{{route('pricing')}}">Please Topup</a></div>
            </div>
        @endforeach --}}
        @php
            $user = json_decode($this->user);
            $permissionTemplateMap = [
                'create_receipt_note' => 'create-goods-receipt',
                'view_sent_receipt_notes_tables' => 'sent-goods-receipt',
                'add_receiver' => 'add-goods-receiver',
                'view_receiver' => 'all-goods-receiver',
                'add_receipt_note_prefix' => 'goods-prefix',
            ];
        @endphp
        ...
        @foreach ($templates as $template)
            @php
                ...
                if (isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission)) {
                    $permissions = $user->team_user->permissions->permission->{'receipt_note'};
                    foreach ($permissionTemplateMap as $permission => $templateName) {
                        // dd($templateName, $templatePageName);    
            ...
            @if ($hasSubUserPermission)
                <li>
                    <a href="{{ route('grn', ['template' => $template->template_page_name]) }}"
