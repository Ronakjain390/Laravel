    ...
    <ul class="space-y-2 ">
        ...
        @php
            ...
            $permissionTemplateMap = [
                ...
                'view_sent_invoice_tables' => 'sent_invoice',
                'view_purchase_orders' => 'purchase_order_seller',
                'view_buyers' => 'all_buyer',
                'add_buyer' => 'add_buyer',
                'invoice_prefix' => 'invoice_series_no',
                'create_quotation' => 'create_quotation',
            ];

            // Rearrange the templates array
            $createInvoice = $templates[0];
            $createEstimate = $templates[6];
            $otherTemplates = array_slice($templates, 1, 5);
            ...
            {{-- @dump($template) --}}
                ...
                @if ($hasSubUserPermission || !isset($user->team_user) || !isset($user->team_user->permissions) || !isset($user->team_user->permissions->permission))
            <li>
                @if($template->template_name === 'Create Invoice')
                    <a href="#"
                       ...
                       wire:click.prevent="checkFeatureUsageLimit"
                    >
                        <span>{{ $template->template_name }}</span>
                    </a>
                    @if($featureUsageLimitExceeded)
                        <div class="mt-2 p-2 text-xs text-red-800 rounded-lg bg-red-100 dark:bg-gray-800 dark:text-red-400" role="alert">
                            Feature usage limit is over or expired. Please upgrade your plan.
                        </div>
                    @endif
                @else
                    <a href="{{ route('seller', ['template' => $template->template_page_name]) }}"
                       class="flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100 rounded-lg items-center dark:focus:ring-gray-500 group"
                    >
                        <span>{{ $template->template_name }}</span>
                    </a>
            ...
            @if(isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission))
                ...
                @if($permissions->view_quotation_tables == 1 && $permissions->view_sent_invoice_tables != 1)
                    <li>
                        <a href="{{ route('seller', ['template' => 'sent_quotation']) }}"
                            class="flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100 rounded-lg items-center dark:focus:ring-gray-500 group">
                            <span class="">Sent Quotation</span>
                        </a>
                    </li>
                @endif
            @endif
