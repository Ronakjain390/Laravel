<div>
    ...
    <div class="rounded-md pb-2 shadow-sm justify-between flex " role="group">
        <div class="flex" >
        ...
        {{-- <button type="button"  wire:click="innerFeatureRedirect('deleted_sent_invoice', null)"
        ...
        {{-- <button type="button" wire:click="innerFeatureRedirect('detailed_sent_challan', null)"
            ...
            View</button>
                ...
                @if ($mainUser->team_user->permissions->permission->seller->export == 1)
        ...
        <a  wire:click="export"
            ...
            class="rounded-r-lg border border-gray-600  px-4 py-1 text-sm  text-black bg-white  focus:z-10 focus:bg-gray-900 focus:text-white focus:ring-2 focus:ring-gray-500 dark:border-white dark:text-white dark:hover:bg-gray-700 dark:hover:text-white dark:focus:bg-gray-700">Export</a>
           @endif --}}
            {{-- <details class="relative mt-2" x-show="selectedProducts.length > 0">
                ...
                <ul class="absolute right-0 bg-white border rounded mt-1 py-1 text-gray-700 whitespace-nowrap shadow text-sm">
    ...
    <div x-data="checkboxes()" x-init="initCheckboxes({{ $invoices->pluck('id') }})" class="min-w-full overflow-auto">
        <div>
            ...
            <div x-data="checkboxes()" x-init="initCheckboxes({{ $invoices->pluck('id') }})" class="min-w-full overflow-auto">
                ...
                <tbody class="text-black"  x-data="{ open: false, modalContent: {} }">
                    @if ($invoices && count($invoices))
                        @foreach ($invoices as $counter => $columnName)
                            @php
                            ...
                            <td class="px-2  text-[0.6rem] border-2 border-gray-300 p-1.5 ">
                                ...
                                <td class="px-2  text-[0.6rem] border-2 border-gray-300 ">{{ $columnName->buyer_details->state ?? null }}</td>
                                <td class="px-2 text-[0.6rem] border-2 border-gray-300  ">
