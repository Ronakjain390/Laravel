    ...
    <ul class="space-y-2 ">
        ...
        @php
            ...
            $permissionTemplateMap = [
                ...
                'view_sent_challans_tables' => 'sent_challan',
                'view_received_return_challans_tables' => 'received_challan',
                'view_receiver' => 'view_receiver',
                'add_receiver' => 'add_receiver',
                'challan_prefix' => 'challan_series_no'
            ];
        @endphp
        ...
        @foreach ($templates as $template)
            @php
                ...
                if (isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission)) {
                    $permissions = $user->team_user->permissions->permission->{'sender'};
            ...
            <li>
                @if($template->template_name === 'Create Challan')
                    <a href="#"
                    ...
                    @endif
                @else
                    <a href="{{ route('sender', ['template' => $template->template_page_name]) }}"
                       class="flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100 rounded-lg items-center dark:focus:ring-gray-500 group"
                    >
                        <span>{{ $template->template_name }}</span>
                    </a>
        ...
        @endforeach
        @if (isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission))
        @if (isset($user->team_user->permissions->permission->{'stock'}) && $user->team_user->permissions->permission->{'stock'}->{'add_stock'} == 1)
         <li>
            <a  href="{{ route('stock') }}"
                class=" flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100  rounded-lg  items-center dark:focus:ring-gray-500 group">
        ...
        @endif
        {{-- Check Auth User is team Member --}}
        @if(Auth::getDefaultDriver() == 'team-user')
            <li>
                <a  wire:click="featureRedirect('sfp_challans', null)"
                    class=" flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100  rounded-lg  items-center dark:focus:ring-gray-500 group">
