<div x-data="invoiceComponent('{{ auth()->user()->state }}', {{ json_encode($panelUserColumnDisplayNames) }}, {{ json_encode($rows) }}, '{{ $context }}', @entangle('selectUser'), {{ json_encode($units) }})"
    ...
    <div>
        ...
        <div
            class="@if($pdfData->challan_templete == 1 && $updateForm == false) grid sm:grid-cols-2
                @elseif($pdfData->challan_templete == 2 || $pdfData->challan_templete == 4) grid sm:grid-cols-2
                @elseif($pdfData->challan_templete == 5 || $pdfData->challan_templete == 6) grid-cols-1
                @elseif($updateForm == false) grid sm:grid-cols-2
         ...
         <!-- Column 1 -->
                ...
                <div
                    ...
                    <div x-data="{ addUser: false, inputsDisabled: true }">
                        <div x-data="{ search: '', selectedUser: null }" wire:ignore.self>
                            ...
                            <div class="relative">
                                <button id="dropdownSearchButton small_outlined" data-dropdown-toggle="dropdownSearch"
                                    ...
                                    class="text-black border border-gray-400 border-solid flex w-full bg-white hover:bg-orange focus:ring-2 focus:outline-none focus:ring-[#372b2b]/50 font-medium rounded-lg text-[0.6rem] sm:text-xs px-5 py-1 text-center items-center justify-center dark:focus:ring-gray-900 dark:hover-bg-[#050708]/30 mr-2 mb-2"
                                    type="button">
                                ...
                                <ul class="h-48 px-3 pb-3 overflow-y-auto text-[0.6rem] sm:text-xs text-gray-700 dark:text-gray-200"
                                    ...
                                    @foreach ($billTo as $user)
                                        <li class="cursor-pointer"
                                            ...
                                            x-on:click="selectedUser = '{{ $user->receiver_name ?? 'Select Receiver' }}'; selectedAddress = '{{ $user->details[0]->address ?? 'Default Address' }}'; console.log('{{ $user->receiver_name ?? 'Receiver Name is Empty' }}'); selectUser = true; selectedUserState = '{{ $user->details[0]->state ?? null }}'; authUserState = '{{ $authUserState }}';">
                                            <div class="flex items-center pl-2 rounded hover:bg-gray-100 cursor-pointer dark:hover:bg-gray-600">
                                                <label wire:ignore
                                                    class="w-full py-1 ml-2 text-[0.6rem] sm:text-xs font-medium text-gray-900 rounded cursor-pointer dark:text-gray-300">{{ $user->receiver_name ?? null }}</label>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
                                <button wire:click="updateField"
                                @click="selectUserFromOthers()"
                                    class="flex w-full items-center p-3 text-[0.6rem] sm:text-xs font-medium text-green-600 border-t border-gray-200 rounded-b-lg bg-gray-50 dark:border-gray-600 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-green-500 hover:underline">
                        ...
                        <div class="w-full text-gray-900 dark:text-white">
                            @if ($updateForm == true)
                                <div class="grid grid-cols-12 " x-show="!addUser">
                                    <!-- Left Column -->
                                    <div class="col-span-12 md:col-span-6">
                                        <div class="grid">
                                            <!-- Phone -->
                                            <div class="flex flex-row">
                                                <div class="w-1/4 sm:w-24 p-2 text-left text-xs font-semibold text-black">
                                                    Phone
                                                </div>
                                                @if ($selectedUser && $selectedUser['phone'])
                                                    <div class="flex-1 p-2 text-left text-xs text-black capitalize">
                                                        {{ $selectedUser['phone'] }}
                                                    </div>
                                                @endif
                                            </div>
                                    ...
                                    <!-- Right Column -->
                                    <div class="col-span-12 md:col-span-6 sm:ml-6">
                                        <div class="grid ">
                                            <!-- Challan No. -->
                                            <div class="flex flex-row items-center">
                                                <div class="w-1/4 p-2 text-left text-xs font-semibold text-black whitespace-nowrap">
                                                    Challan No.
                                                </div>
                                                @if ($selectedUser != null)
                                                    <div class="flex-1 p-2 ml-3 text-xs text-black capitalize flex items-center whitespace-nowrap">
                                                        @if (isset($selectedUser['challanSeries']) && $selectedUser['challanSeries'] == 'Not Assigned')
                                                            <span class="text-red-700">{{ $selectedUser['challanSeries'] }}</span>
                                                        @else
                                                            <span class="text-black">{{ $selectedUser['challanSeries'] ?? null }}</span>
                                                        @endif
                                            ...
                                            <!-- Date -->
                                            <div class="flex flex-row">
                                                <div class="w-1/4 p-2 text-left text-xs font-semibold text-black">
                                                    Date
                                                </div>
                                                <div class="flex-1 p-2 ml-3 text-right text-xs text-[#686464] text-black capitalize">
                                                    <div class="relative max-w-sm">
                                                        <input wire:model.defer="createChallanRequest.challan_date"
                                                        type="date"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-6 "
                                            ...
                                            <input wire:model.defer="createChallanRequest.challan_date" id="datepicker-autohide" datepicker datepicker-autohide type="text" class="bg-gray-50 border border-gray-300 text-gray-900 w-32 h-6 text-[0.6rem] rounded-lg focus:ring-blue-500 focus:border-blue-500 block ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date"> --}}
                                        </div>
                                                </div>
                                            </div>


                                            <!-- Additional Number Permission -->
                                            @if ($additionalNumberPermission)
                                                <div class="flex flex-row">
                                                    <div class="font-semibold mt-2 pl-2 text-black text-left text-xs w-1/4 whitespace-nowrap">
                                                        Add No.
                                                    </div>
                                                    <div class="capitalize flex-1 mt-2 pl-3 text-black text-xs">
                                                        <input placeholder="Add number"  oninput="if(this.value.length > 10) this.value = this.value.slice(0, 10);" wire:model.defer="createChallanRequest.additional_phone_number" class="hsn-box h-5 sm:h-7 w-32 ml-2 rounded bg-white border border-gray-400 border-solid text-[0.6rem] sm:text-xs text-black focus:outline-none" min="0" type="number" max="9999999999" maxlength="10" />
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                <!-- Your add user form goes here -->
                                    <div class="grid gap-2 sm:grid-cols-2">
                                        ...
                                        <div class="flex flex-row mt-2">
                                            ...
                                            <div class="w-2/3">
                                                <input
                                                    ...
                                                    wire:model.lazy="addReceiverData.phone"
                                                    x-data="{}"
                                                    x-on:input="
                                                        $el.value = $el.value.slice(0, 10);
                                                        if ($el.value.length === 10) {
                                                            $wire.set('addReceiverData.phone', $el.value);
                                                        }
                                                    "
                                                    id="phone"
                                                    class="hsn-box h-7 w-full text-xs rounded-lg bg-white border border-gray-400 text-black focus:outline-none @error('addReceiverData.phone') border-red-500 @enderror"
                                                    type="number"
                                                    max="9999999999"
                                                />
                                                <div id="phone_error" class="text-red-500 text-[0.6rem] mt-1"></div>
                                                {{-- @error('addReceiverData.phone')
                                                    <div class="text-red-500 text-[0.6rem] mt-1">{{ $message }}</div>
                                                @enderror --}}
                                                @if($existingUser)
                                        ...
                                        <div class="flex flex-row mt-2 ">
                                            ...
                                            <input @if ($inputsDisabled == true)  @endif
                                            wire:model.defer="addReceiverData.city"
                                            class="hsn-box h-7 w-2/3  text-xs rounded-lg bg-white border border-gray-400 text-black focus:outline-none"
                                            type="text" />
                                        </div>
                        ...
                        @if (
                            ...
                            <div class=" grid-rows-3 ml-20 hidden sm:block ">
                                <div class="flex flex-row">
                                    ...
                                    <dd
                                        class="pl-2 text-[0.6rem] sm:text-xs inline-block  text-black capitalize hidden sm:block">
                                ...
                                @if ($selectedUser != null && !empty($selectedUser['gst']))
                                    <div class="flex flex-row mt-3">
                                        ...
                                        @if ($selectedUser == !null)
                                            <dd class="text-[0.6rem] sm:text-xs ml-2 capitalize text-black">
                                ...
                                @if ($additionalNumberPermission)
                                <div class="flex flex-row mt-3">
                                    <div class="w-24 text-left text-xs font-semibold text-black whitespace-nowrap">
                                        Add No.
                                    </div>
                                    <div class="flex-1  text-xs text-black capitalize">
                                        <input placeholder="Add number" oninput="if(this.value.length > 10) this.value = this.value.slice(0, 10);"  wire:model.defer="createChallanRequest.additional_phone_number" class="hsn-box h-7 w-32 ml-2 rounded-lg bg-white border border-gray-400 border-solid text-[0.6rem] sm:text-xs text-black focus:outline-none" min="0" type="number" maxlength="10" />
                                    </div>
                                </div>
                            @endif
                            </div>
                ...
                <div class="w-full h-full p-2 border border-gray-200 rounded-lg shadow sm:p-4 bg-[#ebebeb]">
                 <div class="items-center">
                     <div class="grid sm:grid-cols-2 gap-3">
                        ...
                        <div class="flex flex-row  items-center">
                            <dt class="mb-1 text-black text-xs dark:text-gray-400 w-24 whitespace-nowrap">Challan No:</dt>
                            @if ($selectedUser != null)
                 ...
                 <div x-data="{ open: false }" class="gap-3 grid pt-2">
                     <div class="flex items-center mt-2 gap-2">
                         <button @click="open = ! open" class="p-1 rounded-full flex bg-[#e5f811] focus:outline-none focus:ring-2 " style="background-color: #e5f811;">
            ...
            @endif
            {{-- @dump($receiverAddress, $selectedUser) --}}
            <!-- Column 2 -->
            @unless ($pdfData->challan_templete == 1 || $pdfData->challan_templete == 5 || $pdfData->challan_templete == 6)
                @if ($updateForm == true)
                    <div class="items-center justify-center h-auto rounded bg-gray-50 dark:bg-gray-800">
                    ...
                    Ship To
                            ...
                            <div x-data="{ search: '', selectedUserDetails: null }" wire:ignore.self>
                                ...
                                <div class="relative">
                                    <button id="dropdownDetailSearchButton outlined"
                                        ...
                                        class=" flex w-full text-black border border-gray-400 border-solid  bg-white focus:ring-2 focus:outline-none focus:ring-[#372b2b]/50 font-medium rounded-lg text-[0.6rem] sm:text-xs px-5 py-1 text-center items-center justify-center dark:focus:ring-gray-900 dark:hover:bg-[#050708]/30 mr-2 mb-2"
                                        type="button">
                                        @if (!$userSelected)
                            ...
                            <div class="grid gap-2">
                                @if ($selectedUser)
                                    @php
                                        $fields = [
                                            'Phone' => $receiverAddress == '' ? $selectedUser['phone'] ?? '' : strtoupper($receiverPhone),
                                            'Email' => $selectedUser['email'] ?? '',
                                            'GST' => $selectedUser['gst'] ?? '',
                                            'Address' => $receiverAddress == '' ? implode(', ', array_filter([$selectedUser['address'] ?? '', $selectedUser['city'] ?? '', $selectedUser['state'] ?? ''])) : ucfirst($receiverAddress)
                                        ];
                                    @endphp
                                ...
                                <div class="p-2 space-y-6 h-80 overflow-y-auto" @click.stop>
                                            <div class="flex space-x-4"  @click.stop >
                                                <div class="flex bg-white dark:bg-gray-900" wire:ignore.self>
                                                    ...
                                                    <div class="mr-2" x-data="{ dropdownOpen: false, search: '' }" wire:key="item-code-filter-{{ now() }}" wire:ignore.self>
                                                        <div class="flex border border-gray-900 rounded-lg text-xs">
                                                            <button @click="dropdownOpen = !dropdownOpen"
                                                                    class="text-black flex whitespace-nowrap bg-white focus:outline-none rounded-lg px-2 py-1 text-center items-center justify-center mr-2"
                                                                    type="button">
                                                                <span>Item Code</span>
                                                                @if (empty($item_code))
                                                                    <svg class="w-2 ml-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                                                ...
                                                                <small class="flex text-xs ml-1 font-bold text-red-500">
                                                                    @if (!empty($item_code))
                                                                        ({{ $item_code }})
                                                                        <span wire:click.stop="updateVariable('item_code', null)" class="cursor-pointer ml-2">X</span>
                                                                    @endif
                                                                </small>
                                                            </button>
                                                        </div>
                                                        ...
                                                        <div x-show="dropdownOpen"
                                                             ...
                                                             class="z-10 bg-white rounded-lg shadow w-100 dark:bg-gray-700 absolute mt-1">
                                                            <div class="p-3">
                                                                <label for="input-group-search" class="sr-only">Search</label>
                                                            ...
                                                            <ul class="h-48 px-3 pb-3 overflow-y-auto text-xs text-gray-700 dark:text-gray-200">
                                                                <li class="cursor-pointer"
                                                                    x-show="search === '' || 'all'.includes(search.toLowerCase())"
                                                                    @click="dropdownOpen = false; $wire.filterVariable('item_code', null)" wire:ignore.self>
                                                                    <div class="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                                                        <label class="w-full py-2 ml-2 text-xs text-gray-900 rounded cursor-pointer dark:text-gray-300">All</label>
                                                                    </div>
                                                                </li>
                                                                @foreach ($item_codes as $code)
                                                                    <li class="cursor-pointer"
                                                                        x-show="search === '' || '{{ strtolower($code) }}'.includes(search.toLowerCase())"
                                                                        @click="dropdownOpen = false; $wire.filterVariable('item_code', '{{ $code }}')">
                                                                        <div class="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                                                            <label class="w-full py-2 ml-2 text-xs text-gray-900 rounded cursor-pointer dark:text-gray-300">{{ $code }}</label>
                                                                        </div>
                                                                    </li>
                                                                @endforeach
                                                            </ul>
    ...
    @endif
    <div id="errorModal" style="display: none;" x-data x-init="
    ...
    <div x-data="{
            ...
            featureUsageLimitExceeded: @entangle('featureUsageLimitExceeded'),
        ...
        x-init="
            ...
            $watch('featureUsageLimitExceeded', value => console.log('featureUsageLimitExceeded changed:', value));
        "
        ...
        <div x-cloak>
            ...
            <div>
              ...
              x-show="draft && !challanSaved" id="add"
              @click.prevent="if(validateFields()) { draftData(); draft = true; dataChanged = true; } else { /* handle validation failure */ }"
              x-bind:disabled="dataChanged"
              :class="dataChanged ? 'bg-gray-300 text-black' : (selectUser ? 'bg-gray-900 text-white' : 'bg-gray-300 text-black')"
              class="rounded-full btn-size lg:px-8 px-4 py-2 font-medium text-sm">
              Draft
          </button>
              ...
              @if ($mainUser->team_user != null)
                  ...
                  <button type="button"
                      x-show="saved && !featureUsageLimitExceeded"
                      @click.prevent="draft = true; saved = false; $wire.challanEdit()"
                      class="rounded-full btn-size font-medium text-sm @if ($challanSaved) bg-gray-900 text-white @else bg-gray-900 text-white @endif lg:px-8 px-4 py-2 text-black">
                      Edit
                  </button>
                  @endif
              @else
              ...
              @if($teamMembers != null)
                <button
                @click="$wire.emit('openSfpModal',{ challanId: challanId, type: 'challan_id' })"
                    x-show="saved && !featureUsageLimitExceeded"
                    class="rounded-full btn-size font-medium text-sm bg-gray-900 text-white lg:px-8 px-4 py-2">
                    SFP
                </button>
            @endif
            ...
            <form method="dialog" class="h-full">
                ...
                <div class="flex justify-end">
                    <button type="button" wire:click='sfpChallan'
                        class="text-white btn btn-sm mt-6 btn-circle btn-ghost bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs border-2 border-gray-300 px-10 py-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Send</button>
                </div>
        </div>
    </dialog>
</div>
...
<script>
    document.addEventListener('livewire:load', function () {
        Livewire.on('itemIdUpdated', (challanId) => {
            console.log('challanId updated:', challanId);
        });
    });
        window.addEventListener('show-error-message', event => {
