<div>
    ...
    @else
    {{-- <button type="button" wire:click="innerFeatureRedirect('detailed_received_challan', null)"
    ...
    <div x-show="(selectPage || selectAll) && selectedCount > 0" class="bg-blue-100 border-t border-b border-blue-500 text-black text-sm px-4 py-3 my-2">
        ...
        <a href="#" @click.prevent="selectAllItems" class="ml-2 text-blue-600 hover:text-blue-800 underline">Select All</a>
    </div>
      ...
      <table wire:ignore.self class="w-full text-xs text-left text-gray-500 dark:text-gray-400" x-data="{ showSelected: false }">
                ...
                @if (isset($columnName->statuses) && count($columnName->statuses) > 0)
                    ...
                    @if (isset($columnName->pdf_url))
                        <tr ondblclick="window.open('https://docs.google.com/viewer?url={{ urlencode(Storage::disk('s3')->temporaryUrl($columnName->pdf_url, now()->addMinutes(5))) }}', '_blank')"
                            ...
                            @endphp
                                {{ $columnName->statuses[0]->status == 'self_return' || $columnName->statuses[0]->status == 'partially_self_return' ? 'SELF(' . $columnName->receiver . ')' : $columnName->receiver }}
                            </td>
                                            ...
                                            @if (!empty($columnName->statuses) && isset($columnName->statuses[0]))
                                                ...
                                                @if ($columnName->statuses[0]->status != 'sent')
                                                    <p class="whitespace-nowrap" style="font-size: smaller">
                                                        {{ $columnName->created_at->setTimezone('UTC')->format('j F Y, h:i A') }}</p>
                                                @endif
                                            @endif
                                        </div>
                                        <div class="ml-2">
                                    ...
                                    <ul class="py-1 text-[0.6rem] border-2 border-gray-300 text-gray-700 dark:text-gray-200"
                                        ...
                                        @if (isset($columnName->statuses) && count($columnName->statuses) > 0)
                                            ...
                                            @if (in_array($latestStatus->status, ['sent']))
                                                ...
                                                @if ($mainUser->team_user != null)
                                                    @if ($mainUser->team_user->permissions->permission->sender->reject_return_challan == 1)
                                                    <li>
                                                        <a x-data
      ...
      <livewire:components.tag-component :panelId="1" :tableId="2" />
      <livewire:components.comment-component :panelId="1" :tableId="2" />
...
<div x-data="{ isOpen: @entangle('isOpen') }"
    ...
    <div class="bg-white p-6 rounded shadow-lg w-80 sm:w-96">
        <div class="mb-4">
            ...
            <div class="">
                <div class="relative w-full min-w-[200px] h-10 mt-6">
                ...
                <div class="mt-4">
                    <label class="inline-flex items-center mt-3 text-sm">
                        <input type="checkbox" class="form-checkbox h-5 w-5 text-gray-600" wire:model.defer="add_stock_back">
                        <span class="ml-2 text-gray-700 text-sm">Add Qty Back to stock</span>
                    </label>
                </div>
                @endif
            </div>
        ...
        <div class="">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-2">
                ...
                <div class="mt-4">
                    ...
                    <div x-data="{ selectedTags: @entangle('selectedTags'), temporarySelectedTags: @entangle('selectedTags').defer }">
                        @foreach($tags as $tag)
                            <label class="inline-flex items-center mt-3 text-sm">
...
@endif
      ...
      <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
          ...
          <div class="flex flex-wrap items-center justify-end shrink-0 text-blue-gray-500">
              ...
              <button wire:click="sfpReturnChallan"
                      class="middle none center rounded-lg bg-gray-900 py-2.5 px-4 font-sans text-xs   text-white shadow-md transition-all hover:shadow-lg active:opacity-[0.85] disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                 Send
              </button>
          </div>
