    ...
    <ul class="space-y-2 ">
        ...
        @php
            ...
            $permissionTemplateMap = [
                'create_return_challan' => 'create_return_challan',
                'view_sent_challans_tables' => 'sent_return_challan',
                'view_received_challans_tables' => 'received_return_challan',
                'return_challan_prefix' => 'return_challan_series_no'
            ];
        @endphp
        ...
        {{-- @dump($template) --}}
            @php
                ...
                if (isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission)) {
                    $permissions = $user->team_user->permissions->permission->{'receiver'};
            ...
            @if ($hasSubUserPermission && $template->template_page_name !== 'return_challan_series_no')
                <li>
                    <a href="{{ route('receiver', ['template' => $template->template_page_name]) }}"
            ...
            @elseif ((!isset($user->team_user) || !isset($user->team_user->permissions) || !isset($user->team_user->permissions->permission)) && $template->template_page_name !== 'return_challan_series_no')
                <li>
                    <a href="{{ route('receiver', ['template' => $template->template_page_name]) }}"
                    class="flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100 rounded-lg items-center dark:focus:ring-gray-500 group">
                        <span>{{ $template->template_name }}</span>
        ...
        <li>
            <a  wire:click="featureRedirect('sfp_return_challans', null)"
                class=" flex drop-shadow-lg text-sm px-2 text-black font-normal hover:text-white py-2 hover:bg-orange bg-grays border-gray-300 border focus:ring-4 focus:outline-none focus:ring-gray-100  rounded-lg  items-center dark:focus:ring-gray-500 group">
                <span class="">View SFP Return Challan</span>
            </a>
        </li>
        @endif
