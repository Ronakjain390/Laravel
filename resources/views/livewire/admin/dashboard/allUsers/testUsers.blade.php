        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body table-responsive p-0 whitespace-nowrap" style="height: 300px;">
                    ...
                    @endphp --}}
                            @foreach ($paginateLinks as $key => $link)
                                <li>
                                    <button type="button"
                                        @if ($link->url == null) disabled @else wire:click='allUsers({{ $link->label }})' @endif
                                        @if ($currentPage == $link->label) aria-current="page" @endif
                                        class=" @if ($link->url == null) disabled @endif
                                            @if ($currentPage == $link->label) flex items-center justify-center px-3 h-8 text-purple-600 border border-gray-300 bg-purple-50 hover:bg-purple-100 hover:text-purple-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white
                                            @else
                                            flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white @endif
            
                                            "><?php echo $link->label; ?></button>
                                </li>
                            @endforeach
