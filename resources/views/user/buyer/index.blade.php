...
<div class="flex flex-col h-screen bg-[#f2f3f4]">
    <div x-data="{ open: false, menuBarOpen: window.innerWidth > 640, isSmallScreen: window.innerWidth <= 640 }"
         @resize.window="isSmallScreen = window.innerWidth <= 640; if(!isSmallScreen) menuBarOpen = true"
         x-init="() => { if(isSmallScreen) menuBarOpen = false }">

        @livewire('buyer.header.header')
        ...
        <div class="flex flex-col lg:flex-row relative">
            ...
            <aside id="logo-sidebar"
                x-show="menuBarOpen" x-cloak x-transition:enter="ease-out duration-500"
                ...
                <livewire:buyer.sidebar.sidebar :sidenav="Session::get('panel')['feature'] ?? null" />
            </aside>
            ...
            <div id="dynamic-view" class="flex-grow overflow-y-auto" :class="{ 'z-30': isSmallScreen }">
                <div class="flex-1 p-2 sm:ml-64" id="dashboard-body" :class="{ 'sm:ml-64': menuBarOpen, 'sm:ml-0': !menuBarOpen }">
                    @livewire('buyer.content.content', ['features' => Session::get('panel')['feature'] ?? null])
                </div>
            </div>
        </div>
    </div>
