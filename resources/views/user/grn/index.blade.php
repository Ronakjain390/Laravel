...
<div class="flex flex-col h-screen bg-[#f2f3f4]">
    <div x-data="{ open: false , menuBarOpen:true, isSmallScreen: window.innerWidth <= 640 }" @resize.window="isSmallScreen = window.innerWidth <= 640; if(isSmallScreen) menuBarOpen = false" x-init="() => { if(isSmallScreen) menuBarOpen = false }">
        ...
        <div class="flex flex-col lg:flex-row relative">
            ...
            <livewire:grn.sidebar.sidebar :sidenav="Session::get('panel')['feature'] ?? null" />
        </aside>
            ...
            <div id="dynamic-view" class="flex-grow overflow-y-auto" :class="{ 'z-30': isSmallScreen }">
                <div class="flex-1 p-2 sm:ml-64" id="dashboard-body" :class="{ 'sm:ml-64': menuBarOpen, 'sm:ml-0': !menuBarOpen }">
                    @if (Request::path() === 'goods-receipt-note/create-goods-receipt')
                        @livewire('grn.screen.create-goods-receipt')
                    @elseif (Request::path() === 'goods-receipt-note/sent-goods-receipt')
                        @livewire('grn.screen.sent-goods-receipt')
                    @elseif (Request::path() === 'goods-receipt-note/add-goods-receiver')
                        @livewire('grn.screen.add-goods-receiver')
                        @elseif (Request::path() === 'goods-receipt-note/all-goods-receiver')
                        @livewire('grn.screen.view-goods-receiver')
                    @elseif (Request::path() === 'goods-receipt-note/goods-prefix')
                        @livewire('grn.screen.goods-prefix')
                    @elseif (Request::path() === 'goods-receipt-note/sent-receipt-detailed-view')
                        @livewire('grn.screen.sent-receipt-detailed-view')
                    @elseif (Request::path() === 'goods-receipt-note')
                        @livewire('grn.screen.screen', ['features' => Session::get('panel')['feature'] ?? null])
                    @endif
                </div>
            </div>
        </div>
