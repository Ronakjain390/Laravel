@if (in_array($this->persistedTemplate, ['sent_challan']))
    ...
    {{-- <th>
        <div class="flex items-center mb-4">
            ...
            <label for="selectAll" class="ml-2 text-xs font-semibold font-medium text-gray-900 dark:text-gray-300">Select All</label>
        </div>
    </th> --}}
...
<th x-show="selectedCount === 0" scope="col" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold">#</th>
...
{{-- @dump($ColumnDisplayNames) --}}
    ...
    {{-- @dd($columnName); --}}
        ...
        @if ($columnName === 'Invoice No')
            <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold  whitespace-nowrap">
                <button id="dropdownSeriesSearchButton" data-dropdown-toggle="dropdownSeriesSearch"
                    data-dropdown-placement="bottom"
                    class="inline-flex items-center rounded-lg bg-transparent text-center text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    type="button">
                    Invoice No
            ...
            @elseif ($columnName === 'Qty')
                 <th scope="col" x-show="selectedCount === 0" class="va-b px-1 w-0 py-2 text-xs font-semibold whitespace-nowrap">
                    <div class ="flex pr-2" wire:click.stop="sortBy('total_qty')">
                        Qty
            ...
            <th x-show="selectedCount === 0" scope="col" class="font-semibold va-b px-1 flex w-0 py-2 text-xs font-semibold cursor-pointer" wire:click="sortBy('amount')">
               <div> Amount
                <span style="margin-left: 5px;" class="text-[0.6rem] grid grid-rows-1 grid-cols-1">
                    @if($sortField === 'amount')
                        @if($sortDirection === 'asc')
                            <div class="flex flex-col">
                                <span class="text-black leading-none">&#9650;</span>
                                <span class="text-gray-500 leading-none">&#9660;</span>
                            </div>
                        @else
                            <div class="flex flex-col">
                                <span class="text-gray-500 leading-none">&#9650;</span>
            ...
            <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold  whitespace-nowrap">
                ...
                <div x-data="{ search: '', selectedSeries: null }" id="dropdownSeriesSearch"
                    ...
                    <ul class="h-48 px-3 pb-3 overflow-y-auto text-xs text-gray-700 dark:text-gray-200"
                        aria-labelledby="dropdownSeriesSearchButton">
                        {{-- @dump($merged_estimate_series) --}}

                        <li x-show="search === '' || '{{ strtolower('all series' ?? null) }}'.includes(search.toLowerCase())"
                            wire:click="updateVariable('merged_estimate_series', null)" class="cursor-pointer">
                            <div class="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                <label
                                    class=" py-2 ml-2 text-xs  text-black cursor-pointer rounded dark:text-gray-300">All
                                    Series</label>
                        ...
                        @foreach ($merged_estimate_series ?? [] as $key => $series)
                        ...
                        @endforeach
                        {{-- @foreach (json_decode($challanFiltersData)->merged_estimate_series ?? [] as $key => $series)
                ...
                <br>
                @if(isset($estimate_series))
                    <div class="flex w-0 ">
                        <div class="border border-gray-600 font-normal rounded-md px-1 text-[0.6rem]">
                            <small>{{ $estimate_series }}</small>
                            <button wire:click="updateVariable('estimate_series', null)"
                            class="text-red-500 hover:text-red-700 cursor-pointer pr-3">X</button>
                        </div>
                    </div>
                @endif
            ...
            <th x-show="selectedCount === 0" scope="col" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold  whitespace-nowrap">
                <button id="dropdownSeriesSearchButton" data-dropdown-toggle="dropdownSeriesSearch"
                    data-dropdown-placement="bottom"
                    class="inline-flex items-center rounded-lg bg-transparent text-center text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    type="button">
        ...
        @elseif ($columnName === 'Date' || $columnName === 'Sent Date' )
            <th x-show="selectedCount === 0" scope="col" class="font-semibold va-b px-1 w-0 py-2 text-xs">
                <div class="relative" x-data="{
                    ...
                    $watch('$wire.from', value => from = value);
                    $watch('$wire.to', value => to = value);
                ">
                    <button id="dropdownDateSearchButton" @click="isOpen = !isOpen"
                        class="inline-flex whitespace-nowrap items-center rounded-lg bg-transparent text-center text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                        type="button">
                    ...
                    <div wire:ignore.self id="dropdownDateSearch" x-show="isOpen"
                        class="z-10 w-40 bg-white absolute rounded-lg shadow w-100 dark:bg-gray-700"
                        @click.away="isOpen = false">
                        <div class="p-3">
                            <label for="date-from" class="text-xs text-black dark:text-gray-300">From</label>
                            <input x-model="from" type="date" id="date-from"
                                wire:model="from"
                                wire:change="updateVariable('from', $event.target.value)"
                                @change="if(to) { isOpen = false; $wire.call('updateVariable', 'to', to); }"
                                class="bg-gray-50 p-1 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full">
                        </div>
                        <div class="p-3">
                            <label for="date-to" class="text-xs text-black dark:text-gray-300">To</label>
                            <input x-model="to" type="date" id="date-to"
                                wire:model="to"
                                wire:change="updateVariable('to', $event.target.value)"
                                @change="if(from) { isOpen = false; $wire.call('updateVariable', 'from', from); }"
                                class="bg-gray-50 p-1 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full">
                        </div>
                    </div>
                    <div x-show="from || to" class="flex w-0 whitespace-nowrap">
                        <div class="border border-gray-600 grid font-normal rounded-md px-1 text-[0.6rem]">
                            <small x-text="from ? new Date(from).toLocaleDateString() : ''"></small>
                            {{-- <small x-show="to"> - </small> --}}
                            <small x-text="to ? new Date(to).toLocaleDateString() : ''"></small>
                        </div>
                            <button @click="reset()" class="text-red-500 hover:text-red-700 cursor-pointer pl-3">X</button>
                    </div>
                </div>
            </th>
            @elseif ($columnName ===  'Received Date' )
            <th x-show="selectedCount === 0" scope="col" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold">
                <div class="relative">
                    <button id="dropdownDateSearchButton" data-dropdown-toggle="dropdownReceivedDateSearch"
                        data-dropdown-placement="bottom"
                        class="inline-flex whitespace-nowrap items-center rounded-lg bg-transparen text-center text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                        type="button">
                        Received Date
                        <svg class="ml-2.5 h-2.5 w-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                            fill="none" viewBox="0 0 10 6">
        ...
        <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold " x-data="{ search: '', selectedReceiver: null, isOpen: false }">
            ...
            <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold ">
                ...
                <div x-data="{ search: '', selectedBuyer: null }" id="dropdownBuyerSearch"
                    ...
                    <li x-show="search === '' || '{{ strtolower($buyer ?? '') }}'.includes(search.toLowerCase())"
                        ...
                        <div class="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                            <label class="py-2 ml-2 text-xs text-black cursor-pointer rounded dark:text-gray-300">{{ $buyer ?? '' }}</label>
                        </div>
                    </li>
                    @endforeach
                    </ul>
                ...
                <div class="flex  whitespace-nowrap">
                    @php
                        $routeName = $this->persistedTemplate;
                        if ($routeName === 'sent-goods-receipt') {
                            $buyer = App\Models\GoodsReceipt::where('receiver_goods_receipts_id', $buyer_id)->first();
                            $buyerName = $buyer->receiver_goods_receipts ?? '';
                        } else {
                            $buyer = App\Models\Invoice::where('buyer_id', $buyer_id)->first();
        ...
        @elseif ($columnName === 'Status' || $columnName === 'Sent Status')
                ...
                <div x-data="{ search: '', selectedStatus: null }" id="dropdownStatusSearch"
                    ...
                    <ul class="h-48 px-3 pb-3 overflow-y-auto text-xs text-gray-700 dark:text-gray-200"
                        ...
                        {{-- @foreach (json_decode($challanFiltersData)->status ?? [] as $key => $status) --}}
                            @php
                                $statuses = ['Sent' => 'sent', 'Accepted' => 'accept', 'Created' => 'created', 'Rejected' => 'reject', 'Draft' => 'draft'];
                            @endphp
            ...
            @endif
        @else
            <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold capitalize whitespace-nowrap">{{ $columnName }}</th>
        @endif
    @endforeach
@endif
{{-- @dd($this->persistedTemplate); --}}

@unless (in_array($this->persistedTemplate, [
    'detailed_sent_invoice',
    'detailed_all_buyers',
    'detailed_purchase_order_buyer',
    'detailed_received_challan',
    'detailed_received_return_challan',
    'detailed_purchase_order',
    'detailed_sent_challan',
    'detailed_sent_return_challan',
    'sent-receipt-detailed-view',
    'deleted_sent_challans',
    'deleted_sent_invoice'
]))
    <th scope="col" x-show="selectedCount === 0" class="font-semibold va-b px-1  w-0 py-2 text-xs font-semibold capitalize">Actions</th>
@endunless
