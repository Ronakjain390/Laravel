<div class="max-w-full mt-10 mb-4 border-b border-gray-200 dark:border-gray-700 p-6 rounded-lg bg-[#e9e6e6] dark:bg-gray-800"">
    ...
    <div id="myTabContent" class=" p-4 rounded-lg dark:bg-gray-800">
        ...
        <div class="grid gap-4">
            ...
            <div class="flex items-center"> <!-- Use flex container to align items horizontally -->
            ...
            <div class="flex justify-between ">
                ...
                <button type="button" wire:click="invoiceTermsAndConditions" class="rounded-full bg-[#28a745] px-8 py-2 mt-2 text-white hover:bg-yellow-200 hover:text-black">Add</button>
            </div>
        </div>
                    ...
                    <td class="">
                        ...
                        <div id="dropdown-{{ $index }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 border-2">
                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton-{{ $index }}">
                                <li>
                                    <a href="javascript:void(0);" {{-- wire:click="selectInvoiceSeries('{{ $item->id ?? '' }}','{{ $item->series_number ?? '' }}','{{ $item->valid_till ?? '' }}','{{ $item->valid_from ?? '' }}', '{{ $item->assigned_to_name ?? '' }}' )" --}} wire:click="selectInvoiceTerms('{{ json_encode($item) ?? '' }}')" data-modal-target="edit-modal" data-modal-toggle="edit-modal" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                </li>
                                <li>
    ...
    <!-- Default Modal -->
    <div id="edit-modal" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full" wire:ignore.self>
