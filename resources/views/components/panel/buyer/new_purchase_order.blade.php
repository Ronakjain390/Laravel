{{-- <livewire:buyer.content.test-purchase-order> --}}
    <div x-data="invoiceComponent('{{ auth()->user()->state }}', {{ json_encode($panelUserColumnDisplayNames) }}, {{ json_encode($rows) }}, '{{ $context }}', @entangle('selectUser'))"
        x-init="init()" class="rounded-lg dark:border-gray-700 mt-4">
 <!-- Add Row Button -->
        @php
                            ...
                            <div class="col-span-10">
                                <div class="grid gap-2">
                                    ...
                                    <div class="flex flex-row">
                                        ...
                                        <dd
                                            class="text-xs  capitalize @if (isset($selectedUser['purchase_order_series'])) @if ($selectedUser['purchase_order_series'] == 'Not Assigned') text-red-700 @else text-black @endif
                                        @else
                                        text-black @endif">

                                        @if ($selectedUser != null)
                                        <div class="flex-1 ml-2 text-xs text-black capitalize flex items-center whitespace-nowrap">

                                            <span class="text-black">{{ $selectedUser['purchase_order_series'] ?? null }}</span>

                                            <span class="inline-flex items-center">
                                                {{ '-' }}
                                                @livewire('series-number-input', ['challanSeries' => $selectedUser['purchase_order_series'], 'seriesNumber' => $selectedUser['seriesNumber'] , 'method' => 'purchase_order'])
                                            </span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex flex-row ">
                                        ...
                                        <dd class="pl-2 text-xs  text-black capitalize">
                                            {{ Auth::guard(Auth::getDefaultDriver())->user()->email ?? null }}
                        ...
                        <div x-data="{ search: '', selectedUserDetails: null }" id="dropdownDetailSearch"
                            ...
                            <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200"
                                ...
                                <li class="cursor-pointer"
                                            ...
                                            wire:click.prevent="selectUser('{{ $user->series_number->series_number ?? 'Not Assigned' }}', '{{ $user->details[0]->address ?? null }}', '{{$user->details[0]->city ?? null }}', '{{ $user->details[0]->state ?? null}}' , '{{ $user->user->email ?? null }}', '{{ $user->details[0]->phone ?? null }}', '{{ $user->details[0]->gst_number ?? null }}','{{ $user->seller_name ?? 'Select Seller' }}', '{{ json_encode($user ?? null) }}')"
                                            x-on:click="selectedUser = '{{ $user->seller_name ?? 'Select Receiver' }}';  selectedUserState = '{{ $user->details[0]->state ?? '' }}'; console.log('{{ $user->seller_name ?? 'Receiver Name is Empty' }}'); selectUser = true; authUserState = '{{ $authUserState }}';">
                                            <div
        ...
        class="flex justify-center space-x-4 lg:text-lg text-[0.6rem] sm:text-xs m-2">
            ...
            {{-- SFP Button --}}
            <button
            wire:click="$emit('openSfpModal', { challanId: {{ $purchaseOrderId ?? 'null' }}, type: 'purchase_order_id' })"
            @if ($inputsResponseDisabled == true) disabled="" @endif
        ...
        <livewire:components.sfp-component :panelType="'po'"/>
</div>
