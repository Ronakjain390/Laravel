<div class="max-w-2xl mx-auto">
    ...
    <script>
        ...
        <div class="sm:flex items-center mb-4">
            ...
            <button
                ...
                wire:click="$set('activeTab', 'tab2')" {{ $activeTab === 'tab2' ? 'disabled' : '' }}>Receiver Code</button>
            <button
                class="px-4 p-1.5 w-auto text-center font-medium {{ $activeTab === 'tab3' ? 'bg-orange text-white rounded-lg' : '' }}"
                wire:click="$set('activeTab', 'tab3')" {{ $activeTab === 'tab3' ? 'disabled' : '' }}>Bulk Receiver</button>
                        ...
                        <div>
                            ...
                            <div class="grid gap-4">
                                <div>
                                    ...
                                    <input type="text" wire:model.defer="addReceiverData.receiver_name"
                                        wire:ignore.self id="receiver_name" name="receiver_name"
                                        class="mt-1 p-2 h-8 block text-xs w-full text-black rounded-md bg-white border-transparent focus:"
                                        placeholder="Enter Receiver Name">
                                        @error('addReceiverData.receiver_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model.defer="addReceiverData.address" wire:ignore.self
                                        ...
                                        @error('addReceiverData.address') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model.defer="addReceiverData.pincode"
                                        wire:keydown.enter="cityAndStateByPincode" wire:ignore.self id="pincode"
                                        name="pincode"
                                        class="mt-1 p-2 h-8 block text-xs w-full text-black rounded-md bg-white border-transparent focus:border-gray-500 "
                                        placeholder="Area Pincode"
                                        oninput="if (this.value.length === 6) { @this.set('addReceiverData.pincode', this.value); @this.call('cityAndStateByPincode'); }">
                                        @error('addReceiverData.pincode') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="city" class="block text-sm text-black">City <span
                                            class="text-red-600">*</span></label>
                                    <input type="text" wire:model.defer="addReceiverData.city" wire:ignore.self
                                        id="city" name="city"
                                        class="mt-1 p-2 h-8 block text-xs w-full text-black rounded-md bg-white border-transparent focus:border-gray-500 "
                                        placeholder="City">
                                        @error('addReceiverData.city') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="state" class="block text-sm text-black">State <span
                                            class="text-red-600">*</span></label>
                                    <input type="text" wire:model="addReceiverData.state" wire:ignore.self
                                        id="state" name="state"
                                        class="mt-1 p-2 h-8 block text-xs w-full text-black rounded-md bg-white border-transparent focus:border-gray-500 "
                                        placeholder="State">
                                        @error('addReceiverData.state') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
        ...
        @elseif ($activeTab === 'tab2')
            <div id="myTabContent" class=" p-4 rounded-lg dark:bg-gray-800 ">
                <div class="mt-2" role="tabpanel">
                    <div>
                        <label for="receiver_code" class="block text-sm text-black">Receiver Code</label>
                        <input type="text" wire:model="addReceiverData.receiver_special_id" id="receiver_special_id"
                            name="receiver_special_id"
                            class="mt-1 p-2 h-8  block w-full rounded-md bg-white border-transparent text-black focus:border-gray-500"
                            placeholder="If Receiver is TheParchi User">
                        @error('addReceiverData.receiver_special_id')
                    ...
                    <div class="flex justify-center">
                        <button type="button" wire:click="callAddReceiver" id="add"
                ...
                <div class="mt-2"  role="tabpanel" >
                    <div class="bg-white border border-gray-300 rounded-lg p-2 shadow-md">
                        <div class="text-blue-700 font-semibold mb-2 flex flex-col md:flex-row justify-between items-center">
                            <a href="{{ route('receiver.exportColumns') }}"
    ...
    @if ($activeTab === 'tab1')
        <div class="max-w-5xl mt-4 mx-auto " id="receiver-manually" role="tabpanel"
            ...
            <div wire:ignore.self>
                ...
                <div class="{{ $showInputBoxes ? '' : 'hidden' }}relative mt-2">
                    ...
                    <div id="inputBoxes" class="hidden mt-2 bg-[#e9e6e6] p-6 rounded-lg">
                        ...
                        <div>
                            <label for="organisation_type" class="block text-sm text-black mt-2">Organisation
                                Type</label>

                            <input type="text" wire:model.defer="addReceiverData.organisation_type"
            ...
            <div class="flex justify-center">
                <button type="button" wire:click="validateAndReceiver"
