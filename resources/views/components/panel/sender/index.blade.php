            ...
            <div class="flex flex-col items-center">
                ...
                <div class="w-full mb-6">
                    <div class="flex mt-6 space-x-3 md:mt-6 w-full">
                        ...
                        @if (isset($user->team_user->permissions->permission->{'sender'}) && $user->team_user->permissions->permission->{'sender'}->create_challan == 1)
                        ...
                        <button type="button"
                                ...
                                @else
                                    wire:click="innerFeatureRedirect('create_challan', '1')"
                                @endif
                    ...
                    @if (isset($user->team_user) && isset($user->team_user->permissions) && isset($user->team_user->permissions->permission))
                    @if (isset($user->team_user->permissions->permission->{'sender'}) && $user->team_user->permissions->permission->{'sender'}->add_receiver == 1)
                    <div class="flex mt-6 space-x-3 md:mt-6 w-full">
                        <button type="button"
                                @if (is_null($user->address) || is_null($user->pincode) || is_null($user->state) || is_null($user->city))
                                    onclick="window.location.href = '{{ route('profile') }}'; return false;"
                                @else
                                    wire:click="innerFeatureRedirect('add_receiver', '6')"
                                @endif
