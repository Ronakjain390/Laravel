<div class="relative overflow-x-auto shadow-md sm:rounded-lg border h-screen">


    <table class="w-full text-xs text-left text-gray-500 dark:text-gray-400 ">
        <thead class="text-xs text-black bg-gray-300 dark:bg-gray-700 dark:text-gray-400">
            {{-- <tr class="whitespace-nowrap"> --}}
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                #
            </th>

            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Receiver Name
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Special Id
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Address
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                State
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Pincode
            </th>

            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Email
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Phone
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Pancard
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                GST
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Added On
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Type
            </th>
            <th scope="col" class="va-b px-2 py-2 capitalize whitespace-nowrap">
                Action
            </th>
            {{-- </tr> --}}
        </thead>
        {{-- @dd($receiverDatas); --}}
        @foreach ($receiverDatas as $index => $data)
        {{-- @dd($data->user->added_by) --}}
        @php
            $data = (object) $data;
            // dd($data);
        @endphp
            <tbody>
                <tr
                    class="@if ($index % 2 == 0) border-b bg-[#e9e6e6] dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600 @else bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-600 @endif whitespace-nowrap text-black h-0 w-0 ">
                    <td class="px-2">
                        {{ $index + 1 }}
                    </td>
                    <td class="px-2">
                        {{ $data->receiver_name }}
                    </td>
                    <td class="px-2 ">
                        {{ $data->receiver_special_id }}
                    </td>
                    <td class="px-2 ">
                        <p> {{ $data->user->address ?? '' }} </p>
                    </td>

                    <td class="px-2 ">
                        {{ $data->user->state ?? '' }}
                    </td>
                    <td class="px-2 ">
                        {{ $data->user->pincode ?? '' }}
                    </td>
                    <td class="px-2 ">
                    </td>

                    <td class="px-2 ">
                        {{ $data->user->phone ?? '' }}
                    </td>
                    <td class="px-2 ">
                    </td>
                    <td class="px-2 ">
                        {{ $data->user->gst_number ?? '' }}
                    </td>
                    <td class="px-2">
                        {{ date('j/m/Y', strtotime($data->created_at)) }}
                    </td>

                    <td class="px-2 ">
                        @if(isset($data->user->added_by))
                        Manual
                        @else
                        TP-User
                        @endif
                    </td>

                    <td class="">
                        <button id="dropdownDefaultButton-{{ $index }}"
                            data-dropdown-toggle="dropdown-{{ $index }}"
                            class="text-white bg-gray-900 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-2.5 py-1.5 m-1 text-center inline-flex items-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                            type="button">Select <svg class="w-2.5  ml-2.5" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2" d="m1 1 4 4 4-4" />
                            </svg></button>
                        <div id="dropdown-{{ $index }}"
                            class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 border-2">
                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                aria-labelledby="dropdownDefaultButton-{{ $index }}">
                                @if(isset($data->user->added_by))
                                @if($data->user->added_by == $data->user_id)
                                <li>
                                    <a href="#" wire:click="selectReceiver({{ json_encode($data) }} )"
                                        onclick="my_modal_1.showModal()"
                                        class="block px-4 py-2 hover:bg-orange dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                </li>
                                @endif
                                @endif
                                <li>
                                    <a href="javascript:void(0);"
                                        wire:click="$emit('triggerDelete', {{ $data->id }})"
                                        class="block px-4 py-2 hover:bg-orange dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                </li>

                            </ul>
                        </div>
                    </td>
                </tr>
            </tbody>
        @endforeach


    </table>

</div>

<dialog id="my_modal_1" class="modal" wire:ignore.self>
    @if (session()->has('message'))
    <div class="fixed inset-x-0 top-0 z-50 p-2 bg-green-500 text-white text-center">
        {{ session('message') }}
    </div>
    @endif
    <div class="modal-box bg-white max-w-4xl flex flex-col">
        <!-- Fixed Header -->
        <div class="sticky top-0 bg-white z-10 border-b border-gray-400 p-4">
            <h3 class="text-lg font-bold text-black">Edit Series Number</h3>
        </div>

        <!-- Scrollable Body -->
        <div class="flex-1 overflow-y-auto p-4">
            <div class="pt-5">
                <div>
                    <label for="receiver_name" class="block text-black font-medium">Receiver Name</label>
                    <input @if ($selectReceiver['added_by'] !== null) disabled @endif
                        wire:model.prevent="selectReceiver.receiver_name" type="text"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                </div>
                <!-- Addresses Section -->
                <div>
                    {{-- <label for="receiver_name" class="block text-black font-medium">Address</label> --}}
                    <!-- Add Address Button at the Top -->
                    <div wire:key="detail" x-data="defaultAddress(@entangle('selectReceiver'))" class="mt-4 border rounded-lg p-4 border-gray-300 relative">
                        <input type="hidden" wire:model.defer="selectReceiver.id">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="location_name" class="block text-sm font-medium text-black">Location Name</label>
                                <input x-model.lazy="selectReceiver.location_name" wire:model.defer='selectReceiver.location_name' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="address" class="block text-sm font-medium text-black">Address</label>
                                <input x-model.lazy="selectReceiver.address" wire:model.defer='selectReceiver.address' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="phone" class="block text-sm font-medium text-black">Phone</label>
                                <input @if ($inputsDisabled == true) disabled @endif
                                    x-model.lazy="selectReceiver.phone"
                                    wire:model.defer='selectReceiver.phone'
                                    type="number"
                                    max="9999999999" maxlength="10"
                                    oninput="if(this.value.length > 10) this.value = this.value.slice(0, 10);"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="gst_number" class="block text-sm font-medium text-black">GST Number</label>
                                <input x-model.lazy="selectReceiver.gst_number" wire:model.defer='selectReceiver.gst_number' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="pincode" class="block text-sm font-medium text-black">Pincode</label>
                                <input x-model.lazy="selectReceiver.pincode" wire:model.defer='selectReceiver.pincode' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="state" class="block text-sm font-medium text-black">State</label>
                                <input x-model.lazy="selectReceiver.state" wire:model.defer='selectReceiver.state' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="city" class="block text-sm font-medium text-black">City</label>
                                <input x-model.lazy="selectReceiver.city" wire:model.defer='selectReceiver.city' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="bank_name" class="block text-sm font-medium text-black">Bank Name</label>
                                <input x-model.lazy="selectReceiver.bank_name" wire:model.defer='selectReceiver.bank_name' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="branch_name" class="block text-sm font-medium text-black">Branch Name</label>
                                <input x-model.lazy="selectReceiver.branch_name" wire:model.defer='selectReceiver.branch_name' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="bank_account_no" class="block text-sm font-medium text-black">Bank Account No</label>
                                <input x-model.lazy="selectReceiver.bank_account_no" wire:model.defer='selectReceiver.bank_account_no' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="ifsc_code" class="block text-sm font-medium text-black">IFSC Code</label>
                                <input x-model.lazy="selectReceiver.ifsc_code" wire:model.defer='selectReceiver.ifsc_code' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                            <div>
                                <label for="tan" class="block text-sm font-medium text-black">TAN</label>
                                <input x-model.lazy="selectReceiver.tan" wire:model.defer='selectReceiver.tan' type="text"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            </div>
                        </div>
                        <button type="button" wire:click="saveDefaultAddress"
                            class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                            :class="{
                                'bg-gray-400 dark:bg-gray-300': !isFormValid(),
                                'bg-gray-700 hover:bg-gray-800 dark:bg-gray-700 dark:hover:bg-gray-800': isFormValid()
                            }"
                            :disabled="!isFormValid()">
                            Save
                        </button>
                    </div>
                    <!-- Address Cards -->
                     <!-- Address Cards -->
    <div id="address-fields">
        @foreach($otherAddresses as $index => $address)
             <div wire:key="address-{{ $index }}" x-data="defaultAddress(@entangle('otherAddresses').defer, {{ $index }})" class="mt-4 border rounded-lg p-4 border-gray-300 relative">
                <input type="hidden" wire:model.defer="otherAddresses.{{ $index }}.id">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="location_name" class="block text-sm font-medium text-black">Location Name</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.location_name" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <!-- Add more fields for each address -->
                    <div>
                        <label for="address" class="block text-sm font-medium text-black">Address</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.address" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <!-- Repeat for other fields like phone, gst_number, etc. -->
                    <div>
                        <label for="phone" class="block text-sm font-medium text-black">Phone</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.phone" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="gst_number" class="block text-sm font-medium text-black">GST Number</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.gst_number" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="pincode" class="block text-sm font-medium text-black">Pincode</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.pincode" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="state" class="block text-sm font-medium text-black">State</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.state" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="city" class="block text-sm font-medium text-black">City</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.city" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="bank_name" class="block text-sm font-medium text-black">Bank Name</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.bank_name" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="branch_name" class="block text-sm font-medium text-black">Branch Name</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.branch_name" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="bank_account_no" class="block text-sm font-medium text-black">Bank Account No</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.bank_account_no" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="ifsc_code" class="block text-sm font-medium text-black">IFSC Code</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.ifsc_code" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="tan" class="block text-sm font-medium text-black">TAN</label>
                        <input  wire:model.defer="otherAddresses.{{ $index }}.tan" type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                </div>
                <button type="button" wire:click="saveAddress({{ $index }})"
                    class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                    >
                    Save
                </button>
            </div>
        @endforeach
    </div>
                    {{-- <button type="button"
                        class="hover:bg-gray-300 text-gray-500 font-medium rounded-lg text-sm px-5 py-2 m-2 text-center inline-flex items-center"
                        wire:click.prevent='addNewReceiverDetail'> + Add more address
                    </button> --}}

                </div>
                <!-- Add other fields (phone, gst_number, state, city, bank_name, etc.) here -->
            </div>
        </div>

        <!-- Fixed Footer -->
        <div class="sticky bottom-0 bg-white z-10 border-t border-gray-400 p-2 text-right">
            <form method="dialog">
                <button onclick="window.location.reload();" class="px-4 py-2.5 mr-1 font-sans text-xs text-red-500 transition-all rounded-lg middle none center hover:bg-red-500/10 active:bg-red-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                    Cancel
                </button>
                {{-- <button wire:click.prevent="updateReceiverDetail"
                    class="px-4 py-2.5 mr-1 font-sans text-xs text-blue-500 transition-all rounded-lg middle none center hover:bg-blue-500/10 active:bg-blue-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                    Save Changes
                </button> --}}
            </form>
        </div>
    </div>
</dialog>

<!-- Default Modal -->
<div id="medium-modal" tabindex="-1"
    class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full"
    wire:ignore.self>
    <div class="relative w-full max-w-6xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <!-- Modal header -->
            <div class="flex items-center justify-between p-3 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">
                    Edit Receiver
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="medium-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                        </svg>
                        <span class="sr-only">Close modal</span>
                    </button>
                </h3>
            </div>
            <!-- Modal body -->
            <div class="p-6 space-y-6">
                <div>
                    <label for="receiver_name" class="block text-md font-medium">Receiver Name</label>
                    <input @if ($selectReceiver['added_by'] !== null) disabled @endif
                        wire:model.prevent="selectReceiver.receiver_name" type="text"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                </div>
                <!-- Addresses Section -->
                <div>
                    <label class="block text-md font-medium">Addresses</label>
                    <!-- Add Address Button at the Top -->
                    {{-- <button type="button"
                        class="text-white bg-gray-900 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2 m-2 text-center inline-flex items-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                        wire:click.prevent='addNewReceiverDetail'>Add Address</button> --}}
                    {{-- id="add-address-button" --}}
                    <!-- Address Cards -->
                    <div id="address-fields">
                        <!-- Example Address Card with Delete Button -->
                        {{-- @foreach ($selectReceiver['details'] as $key => $detail)
                            <div wire:key="detail-{{ $key }}"
                                class="mt-4 border rounded-lg p-4 border-gray-300 relative">
                                <button type="button"
                                    class="text-white bg-gray-900 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-2 py-1 absolute top-1 right-1 text-center inline-flex items-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                                    wire:click.prevent="removeReceiverDetail({{ $key }})">Delete</button>
                                <input type="hidden"
                                    wire:model.prevent="selectReceiver.details.{{ $key }}.id">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label for="location_name" class="block text-sm font-medium">Location Name</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.location_name'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="address" class="block text-sm font-medium">Address</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.address'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="phone" class="block text-sm font-medium">Phone</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.phone'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="gst_number" class="block text-sm font-medium">GST Number</label>
                                        <input
                                            wire:model.prevent='selectReceiver.details.{{ $key }}.gst_number'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="pincode" class="block text-sm font-medium">Pincode</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.pincode'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="state" class="block text-sm font-medium">State</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.state'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="city" class="block text-sm font-medium">City</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.city'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="bank_name" class="block text-sm font-medium">Bank Name</label>
                                        <input
                                            wire:model.prevent='selectReceiver.details.{{ $key }}.bank_name'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="branch_name" class="block text-sm font-medium">Branch Name</label>
                                        <input
                                            wire:model.prevent='selectReceiver.details.{{ $key }}.branch_name'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="bank_account_no" class="block text-sm font-medium">Bank Account
                                            No</label>
                                        <input
                                            wire:model.prevent='selectReceiver.details.{{ $key }}.bank_account_no'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="ifsc_code" class="block text-sm font-medium">IFSC Code</label>
                                        <input
                                            wire:model.prevent='selectReceiver.details.{{ $key }}.ifsc_code'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label for="tan" class="block text-sm font-medium">TAN</label>
                                        <input wire:model.prevent='selectReceiver.details.{{ $key }}.tan'
                                            type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm w-full h-10 rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    </div>
                                </div>
                            </div>
                        @endforeach --}}
                    </div>
                </div>
                <!-- Add other fields (phone, gst_number, state, city, bank_name, etc.) here -->
            </div>
            <!-- Modal footer -->
            <div class="flex items-center p-3 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                <button data-modal-hide="medium-modal" wire:click.prevent="updateReceiverDetail" type="button"
                    class="text-white bg-gray-900 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2 m-2 text-center inline-flex items-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Save
                    Changes</button>
                <button data-modal-hide="medium-modal" type="button"
                    class="text-gray-500 bg-white hover:bg-orange focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Close</button>
            </div>
        </div>
    </div>
</div>

<script>
    // JavaScript function to remove an address card
    function removeAddress(button) {
        const addressCard = button.parentElement;
        addressCard.remove();
    }

    function defaultAddress(selectReceiver) {
    return {
        selectReceiver: selectReceiver || {
            location_name: '',
            address: '',
            phone: '',
            pincode: '',
            state: '',
            city: '',
        },
        isFormValid() {
            return this.selectReceiver.location_name && this.selectReceiver.address && this.selectReceiver.phone && this.selectReceiver.pincode && this.selectReceiver.state && this.selectReceiver.city;
        }
    };
}

</script>
<script type="text/javascript">
    document.addEventListener('livewire:load', function(e) {
        @this.on('triggerDelete', id => {
            Swal.fire({
                title: "Are you sure?",
                text: "Are you sure you want to delete?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#6fc5e0",
                cancelButtonColor: "#d33",
                confirmButtonText: "Delete",
            }).then((result) => {
                if (result.value) {
                    @this.call('deleteReceiver', id);
                    console.log('hello');
                } else {
                    console.log("Canceled");
                }
            });
        });
    });
</script>
