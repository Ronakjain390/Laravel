<div class="rounded-lg dark:border-gray-700 mt-4">
    ...
    @endphp
     ...
     <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12">
                            <div class="grid gap-2">
                                ...
                                {{-- @dd($challanSeries); --}}
                                @php
                                    if (is_string($selectedUser)) {
                                        $selectedUser = json_decode($selectedUser);
                                    } elseif (is_array($selectedUser)) {
                                ...
                                <div class="flex flex-row">
                                    ...
                                    <dd
                                        class="text-xs  capitalize @if (isset($selectedUser->series_number->series_number)) @if ($selectedUser->series_number->series_number == 'Not Assigned') text-red-700 @else text-black @endif
                                @else
                                text-black @endif">
                                @if ($selectedUser != null)
                                    <div class="flex-1 ml-2 text-xs text-black capitalize flex items-center whitespace-nowrap">

                                        <span class="text-black">{{ $challanSeries ?? null }}</span>

                                        <span class="inline-flex items-center">
                                            {{ '-' }}
                                            @livewire('series-number-input', ['challanSeries' => $challanSeries, 'seriesNumber' => $selectedUser->series_num, 'method' => 'return_challan'])
                                        </span>
                                    </div>
                                @endif
                                        {{-- -{{ $selectedUser->series_num ?? null }} --}}
                                    </dd>
                                </div>
        ...
        <div class="items-center justify-center h-auto rounded bg-gray-50 dark:bg-gray-800">
            <div class="w-full h-full p-2 border border-gray-200 rounded-lg shadow sm:p-4 bg-[#ebebeb]">
                        ...
                        <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200"
                            ...
                            {{-- @dump($user) --}}
                                ...
                                <li class="cursor-pointer"
                                    ...
                                    wire:click.prevent="selectUser('{{ $user->sender ?? '' }}', '{{ $user->phone ?? null }}', '{{ $user->email ?? null }}', '{{ $user->address ?? null }}', '{{$user->city ?? null}}', '{{ $user->state ?? null }}', '{{ $user->gst_number ?? null }}','{{ $user->sender_id ?? null }}')">
                                    <div
                ...
                <div class="grid gap-2">
                    ...
                    @if(!empty($selectedUser->gst_number))
                    <div class="flex flex-row">
        ...
        <div wire:loading class="fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            <div class="flex justify-center space-x-4">
                ...
                @if ($mainUser->team_user->permissions->permission->receiver->create_return_challan == 1)
                    ...
                    @endif
                        ...
                        @endphp
                        <button
                            @click="$wire.emit('openSfpModal', { challanId: {{ $challanId ?? 'null' }}, type: 'challan_id' })"
                            @if ($inputsResponseDisabled == true) disabled @endif
                            class="rounded-full btn-size @if ($inputsResponseDisabled == true) bg-gray-300 text-black @else bg-gray-900 text-white @endif lg:px-8 px-4 py-2">
                            SFP
                        </button>
            </div>
        </div>
