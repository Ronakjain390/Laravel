...
class Screen extends Component
{
    ...
    public $addReceiver, $storeChallanSeries;
    ...
    public $receiver_name, $company_name, $receiverName, $receiverAddress, $receiverPhone, $challanModifyData, $email, $address, $pincode, $phone, $state, $sfp, $city, $tan, $errorMessage, $successMessage, $showManualReceiverTab, $receiver_special_id, $errors, $statusCode, $message, $fromDate, $toDate, $termsIndexData;
...
public function updateTotal()
    ...
    public function saveRows($requestData)
    {
        ...
        // Check if series_num is null and fetch the default series number
        if (is_null($this->createChallanRequest['series_num'])) {
            ...
            $challanSeries = $this->createChallanRequest['challan_series'] ?? 'Not Assigned';
            ...
            // Update the createChallanRequest with the new series number
            $this->createChallanRequest['challan_series'] = $challanSeries;
            $this->createChallanRequest['series_num'] = $seriesNum;
        }
    ...
    {
        // dd($requestData, $this->challanId);
        $challanId = session('persistedActiveFeature');
        // dd($challanId);
        $request = request();
        // Update the createChallanRequest with the new data
        $this->createChallanRequest['order_details'] = $requestData['order_details'];
        ...
        foreach ($request->order_details as $index => $order_detail) {
            ...
            if (isset($order_detail['columns'])) {
                foreach ($order_detail['columns'] as $column) {
                    if ($column['column_name'] == 'Article' && ($column['column_value'] === null || $column['column_value'] === '')) {
                        $this->addError('article.' . $index, 'Required');
                        $errors = true;
                    }
                }
        ...
        // dd($request->all());
        $request->merge(['challanId' => $challanId]);
        // Create instances of necessary classes
        $ChallanController = new ChallanController;
    ...
    //         $this->reset('barcode');
    //     } else {
    //         $this->barcodeError = $result->message; // Set the error message on failure
    //     }
    // }
    ...
    public function validateAndReceiver()
    {
        $this->validate([
            'addReceiverData.receiver_name' => 'required',
            'addReceiverData.address' => 'required',
            'addReceiverData.pincode' => 'required|numeric|digits:6',
            'addReceiverData.city' => 'required',
            'addReceiverData.state' => 'required',
            'addReceiverData.email' => 'nullable|email|exists:users,email',
            'addReceiverData.phone' => 'nullable|string|size:10|unique:users,phone',
        ], [
            'addReceiverData.receiver_name.required' => 'The buyer name is required.',
            'addReceiverData.address.required' => 'The address is required.',
            'addReceiverData.pincode.required' => 'The pincode is required.',
            'addReceiverData.pincode.numeric' => 'The pincode must be a number.',
            'addReceiverData.pincode.digits' => 'The pincode must be 6 digits.',
            'addReceiverData.city.required' => 'The city is required.',
            'addReceiverData.state.required' => 'The state is required.',
            'addReceiverData.email.email' => 'The email must be a valid email address.',
            'addReceiverData.email.exists' => 'The email already exists.',
            'addReceiverData.phone.string' => 'The phone must be a string.',
            'addReceiverData.phone.size' => 'The phone must be 10 digits.',
            'addReceiverData.phone.unique' => 'The phone number already exists.',
        ]);
        ...
        $this->callAddReceiverManually();
    }

    public function callAddReceiverManually()
    {
        $request = request();
        $request->merge($this->addReceiverData);
        $ReceiversController = new ReceiversController;
        ...
        if ($this->statusCode === 200) {
            ...
            $this->reset(['statusCode', 'message', 'errors', 'validationErrorsJson','addReceiverData']);
        } else {
            $this->errorMessage = json_encode($result->errors);
...
// }
    ...
    public function modifyChallan(Request $request)
    {
        ...
        // $this->createChallanRequest['challan_date'] = Carbon::parse($modifiedDataArray['challan_date'])->format('Y-m-d');
        // Ensure challan_date is assigned and formatted// Ensure challan_date is assigned and formatted

        // Check if series_num is null and fetch the default series number
        if (is_null($modifiedDataArray['series_num'])) {
            $series = PanelSeriesNumber::where('user_id', $userId)
                ->where('default', "1")
                ->where('panel_id', '1')
                ->select('series_number')
            ...
            $challanSeries = $modifiedDataArray['challan_series'];
            if ($challanSeries == 'Not Assigned') {
                $challanSeries = $series->series_number;
            }
            ...
            // Update the modifiedDataArray with the new series number
            $modifiedDataArray['challan_series'] = $challanSeries;
            $modifiedDataArray['series_num'] = $seriesNum;
        }

        // Merge the existing createChallanRequest with the modified data
        $this->createChallanRequest = array_merge($this->createChallanRequest, $modifiedDataArray);
        $this->challanModifyData = json_encode($modifiedDataArray);
        // dd($this->challanModifyData);
        $this->inputsDisabled = false;
        $PanelColumnsController = new PanelColumnsController;
        ...
        $this->ColumnDisplayNames = $this->getColumnDisplayNames($PanelColumnsController, $request, $userId);
        array_push($this->ColumnDisplayNames, 'item code', 'category', 'location','warehouse', 'unit', 'qty', 'rate', 'tax');
        // Pass the modified data array to the initializeRows method
    ...
    public function selfReturnChallanView(Request $request)
    {
        ...
        // Ensure challan_date is visible on the blade
        $this->emit('challanDateUpdated', $this->createChallanRequest['challan_date']);
