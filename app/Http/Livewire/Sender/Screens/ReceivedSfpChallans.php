...
class ReceivedSfpChallans extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $returnChallans = $query->with(['receiverUser', 'statuses', 'receiverDetails', 'orderDetails', 'orderDetails.columns', 'sfpBy', 'tableTags'])
        ->whereHas('sfpBy', function ($query) {
            $query->whereNotNull('challan_id');
        })
        ->whereExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('return_challan_statuses')
                ->whereColumn('return_challan_statuses.challan_id', 'return_challans.id')
                ->where('return_challan_statuses.status', ['sent'])
                ->whereIn('return_challan_statuses.id', function ($subQuery) {
                    $subQuery->select(DB::raw('MAX(id)'))
                        ->from('return_challan_statuses')
                        ->whereColumn('challan_id', 'return_challans.id')
                        ->groupBy('challan_id');
                });
        })
        ->select('return_challans.*')
        ->latest()
        ->paginate(50);

    // The tags query remains unchanged
    $tags = TagsTable::where('panel_id', 1)
        ...
        // dd($returnChallans);
        return view('livewire.sender.screens.received-sfp-challans',compact('returnChallans'))
